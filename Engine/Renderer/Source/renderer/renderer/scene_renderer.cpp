...
namespace retro::renderer
{
	...
	void scene_renderer::initialize(const shared<camera>& camera)
	{
		...
		create_directional_shadow_map(s_scene_renderer_data.m_csm_shadows.m_dir_light_shadow_map_res.x, s_scene_renderer_data.m_csm_shadows.m_dir_light_shadow_map_res.y);
		create_shadow_fbo(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map);
	...
	void scene_renderer::begin_render()
	{
		...
		s_scene_renderer_data.m_lighting_shader->set_float("pointLight.radius", s_scene_renderer_data.m_pointLight.radius);
		s_scene_renderer_data.m_lighting_shader->set_float("pointLight.intensity", s_scene_renderer_data.m_pointLight.intensity);
		s_scene_renderer_data.m_lighting_shader->set_vec_float4("pointLight.color", s_scene_renderer_data.m_pointLight.color);
		s_scene_renderer_data.m_lighting_shader->set_vec_float4("pointLight.position", s_scene_renderer_data.m_pointLight.position);

		s_scene_renderer_data.m_lighting_shader->set_float("directionalLight.intensity", s_scene_renderer_data.m_directional_light.intensity);
		s_scene_renderer_data.m_lighting_shader->set_vec_float4("directionalLight.color", s_scene_renderer_data.m_directional_light.color);
		s_scene_renderer_data.m_lighting_shader->set_vec_float4("directionalLight.direction", s_scene_renderer_data.m_directional_light.direction);
		...
		renderer::bind_texture(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, 8);
		renderer::bind_texture(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, 9);
	...
	{
		return s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map;
	}
	...
	void scene_renderer::create_directional_shadow_map(uint32_t width, uint32_t height)
	{
		...
		glTextureStorage2D(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, 1, GL_DEPTH_COMPONENT32F, width, height);
		...
		glTextureParameteri(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
		glTextureParameteri(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
		glTextureParameterfv(s_scene_renderer_data.m_csm_shadows.m_dir_shadow_map, GL_TEXTURE_BORDER_COLOR, border);
	}

	void scene_renderer::create_shadow_fbo(GLuint shadow_texture)
	{
		glCreateFramebuffers(1, &s_scene_renderer_data.m_csm_shadows.m_shadow_fbo);
		glNamedFramebufferTexture(s_scene_renderer_data.m_csm_shadows.m_shadow_fbo, GL_DEPTH_ATTACHMENT, shadow_texture,
			0);
	...
	void scene_renderer::generate_frame_buffers()
	{
		...
		s_scene_renderer_data.m_shadow_frame_buffer = frame_buffer::create({
			4096, 4096, {
				{
					"shadow", frame_buffer_attachment_format::redint
				}
			}
			});
