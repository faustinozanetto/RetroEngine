...
namespace retro::renderer
{
	...
	lighting_environment::lighting_environment(const shared<texture_cubemap> texture_cubemap)
	{
		...
		shared<vertex_object_buffer> quad_vbo = vertex_object_buffer::create(quad, sizeof(quad));
		quad_vbo->set_layout({
			{FloatVec3, "aPos"},
			{FloatVec2, "aTexCoord"},
		});
		...
		m_equirectangular_to_cubemap_shader->set_mat4("projection", captureProjection);
		...
		for (unsigned int i = 0; i < 6; ++i)
		{
			m_equirectangular_to_cubemap_shader->set_mat4("view", captureViews[i]);
		...
		m_irradiance_convolution_shader->set_mat4("projection", captureProjection);
		...
		for (unsigned int i = 0; i < 6; ++i)
		{
			m_irradiance_convolution_shader->set_mat4("view", captureViews[i]);
		...
		m_prefilter_shader->set_mat4("projection", captureProjection);
		glBindTextureUnit(0, envCubemap);
		...
		for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
		{
			...
			for (unsigned int i = 0; i < 6; ++i)
			{
				m_prefilter_shader->set_mat4("view", captureViews[i]);
	...
	void lighting_environment::render_environment_cube()
	{
		m_skybox_vao->bind();
		glDrawArrays(GL_TRIANGLES, 0, 36);
	}

	void lighting_environment::render_environment_quad()
	{
		m_quad_vao->bind();
