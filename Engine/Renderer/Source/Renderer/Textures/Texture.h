...
namespace Retro::Renderer
{
	...
	struct FTextureSpecification
	{
		...
		FTextureSpecification() = default;

		FTextureSpecification(std::string path, TextureFiltering filtering,
		                      TextureWrapping wrapping) : path(std::move(path)), filtering(filtering), wrapping(wrapping)
		{
		}
	};
