...
{
	open_gl_shader::open_gl_shader(const shader_specification& shader_specification) : asset(
		asset_type::shader)
	{
		logger::line();
		// Parse both vertex and fragment shaders sources from file.
		const std::string& vertexSource = parse_shader_contents_from_file(shader_specification.vertex_path);
		const std::string& fragmentSource = parse_shader_contents_from_file(shader_specification.fragment_path);
		if (!shader_specification.geometry_path.empty()) {
			const std::string& geometry_source = parse_shader_contents_from_file(shader_specification.geometry_path);
	...
	std::string open_gl_shader::get_shader_type_to_string(shader_type type)
	{
		switch (type)
		{
		case fragment: return "fragment_shader";
	...
	std::map<shader_type, std::string> open_gl_shader::process_shader_sources(const std::string& vertex_source,
	...
	void open_gl_shader::compile_shader()
	{
		...
		for (auto& shader : m_shader_sources)
		{
			...
			logger::info("open_gl_shader::compile_shader | Compiled shader type: " + get_shader_type_to_string(shaderType));
