...
namespace retro::renderer
{
    ...
    open_gl_shader::open_gl_shader(const std::string& vertex_path, const std::string& fragment_path,
                                   const std::string& geometry_path): asset(
        asset_type::shader)
    {
        logger::line();
        // Parse both vertex and fragment shaders sources from file.
        const std::string& vertexSource = parse_shader_contents_from_file(vertex_path);
        const std::string& fragmentSource = parse_shader_contents_from_file(fragment_path);
        const std::string& geometry_source = parse_shader_contents_from_file(geometry_path);
        // Assign shader sources.
