...
{
	void open_gl_frame_buffer::generate_color_texture(uint32_t texture_handle, int index, uint32_t width, uint32_t height,
	...
	void open_gl_frame_buffer::reconstruct()
	{
		...
		{
			logger::error("open_gl_frame_buffer::reconstruct | Frame buffer is not complete: " + std::to_string(fboStatus));
		}
