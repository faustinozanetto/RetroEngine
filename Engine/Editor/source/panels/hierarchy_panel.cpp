...
namespace retro::editor
{
	...
	void hierarchy_panel::on_render_panel()
	{
		...
		if (active_scene)
		{
			active_scene->get_actor_registry().each([&](entt::entity actor_handle)
				{
					const actor actor{ actor_handle, active_scene.get() };
	...
	{
		const auto& name_component = actor.get_component<retro::name_component>();
		...
		bool opened = ImGui::TreeNodeEx(reinterpret_cast<void*>(actor.get_actor_handle()), flags, name_component.name.c_str());
