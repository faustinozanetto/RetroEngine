...
namespace retro::editor
{
	...
	void camera_panel::on_render_panel()
	{
		...
		ImGui::Begin("Camera");
		ImGui::CollapsingHeader("Configure Camera");
		if (editor_interface_utils::draw_property("Field of View", m_camera_fov, 1.0f, 120.0f, 0.1f))
		{
			camera->set_fov(m_camera_fov);
		}
		if (editor_interface_utils::draw_property("Near Plane", m_near_plane, 0.001f, 2000.0f, 0.1f))
		{
			camera->set_near_plane(m_near_plane);
		}
		if (editor_interface_utils::draw_property("Far Plane", m_far_plane, 1.0f, 1000.0f, 0.1f))
		{
