...
namespace retro::editor
{
    ...
    void editor_layer::initialize_editor_scene()
    {
        ...
        const shared<renderer::texture>& albedo_tex = retro_application::get_application().get_assets_manager()->
            create_texture({
                "Assets/Models/SciFiHelmet/glTF/SciFiHelmet_BaseColor.png",
                renderer::texture_filtering::linear,
                renderer::texture_wrapping::clamp_edge,
            });
        renderer::material_texture albedo_mat_texture = {
            albedo_tex, true
        };
        const shared<renderer::texture>& normal_tex = retro_application::get_application().get_assets_manager()->
            create_texture({
                "Assets/Models/SciFiHelmet/glTF/SciFiHelmet_Normal.png",
                renderer::texture_filtering::linear,
                renderer::texture_wrapping::clamp_edge,
            });
        renderer::material_texture normal_mat_texture = {
            normal_tex, true
        };
        const shared<renderer::texture>& metal_rough_tex = retro_application::get_application().get_assets_manager()
            ->create_texture({
                "Assets/Models/SciFiHelmet/glTF/SciFiHelmet_MetallicRoughness.png",
                renderer::texture_filtering::linear,
                renderer::texture_wrapping::clamp_edge,
            });
        renderer::material_texture metal_rough_mat_texture = {
            metal_rough_tex, true
        };
        const shared<renderer::texture>& ao_tex = retro_application::get_application().get_assets_manager()->
            create_texture({
                "Assets/Models/SciFiHelmet/glTF/SciFiHelmet_AmbientOcclusion.png",
                renderer::texture_filtering::linear,
                renderer::texture_wrapping::clamp_edge,
            });
        renderer::material_texture ao_mat_texture = {
            ao_tex, true
        };
        const std::map<renderer::material_texture_type, renderer::material_texture> textures = {
            {renderer::material_texture_type::albedo, albedo_mat_texture},
            {renderer::material_texture_type::normal, normal_mat_texture},
            {renderer::material_texture_type::metallic, metal_rough_mat_texture},
            {renderer::material_texture_type::roughness, metal_rough_mat_texture},
            {renderer::material_texture_type::ambient_occlusion, ao_mat_texture}
        };
        const shared<renderer::material>& helmet_mat = retro_application::get_application().get_assets_manager()->
            create_material({textures, glm::vec4(1.0f, 0.23f, 0.5f, 1.0f), 1.0f, 1.0f});
        ...
        auto helmet_model = retro_application::get_application().get_assets_manager()->
                                                                 create_model({
                                                                     "Assets/Models/SciFiHelmet/glTF/SciFiHelmet.gltf"
                                                                 });
        helmet_actor->add_component<name_component>("SciFi Helmet");
