...
uint      cascade_index)
{
    vec2 random_rotation = texture(gRandomAngles, in_world_pos * correction_factor).rg;
...
float CalculateSoftShadows(uint cascade_index)
{
    ...
    float bias = max(0.001 * (1.0 - dot(normalize(in_normal), -directionalLight.direction)), 0.00001);
