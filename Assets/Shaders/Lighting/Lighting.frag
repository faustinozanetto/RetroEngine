...
layout(binding = 8) uniform sampler2D       gShadowMap;
...
float     searchRegionRadiusUV)
{
    ...
    for (int i = 0; i < u_blocker_search_samples; ++i)
    {
        ...
        float shadow_map_depth = texture(gShadowMap, uv + offset).r;
...
float shadowPCF(vec3 FragPos, vec2 uv, float z0, float bias, float filter_radius_uv)
{
    vec2 random_rotation = texture(gRandomAngles, FragPos * correction_factor).rg;
...
float shadowPCSS(vec3 FragPos, vec2 uv, float z, float bias, float z_vs)
{
    // ------------------------
    ...
    findBlocker(accum_blocker_depth, num_blockers, FragPos, uv, z, bias, searchRegionRadiusUV);
    ...
    // STEP 3: filtering
    // ------------------------
    return shadowPCF(FragPos, uv, z, bias, filter_radius);
}
...
vec3 CalculatePointLightPBR(PointLight light, vec3 FragPos, vec3 CamPos, vec3 Albedo, vec3 Normal, float Metallic, float Roughness, vec3 F0) {
    ...
    vec3  light_direction = light.position.xyz - FragPos;
